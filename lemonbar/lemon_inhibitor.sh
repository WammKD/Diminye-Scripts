...
echo "inhibitor%{A:if [ $(xset -q | grep 'DPMS is' | awk '{ print $3 }') = 'Enabled' ]; then xset -dpms; else xset +dpms; fi; lemon_inhibitor.sh:}%{U#7e57c2}%{+u}%{F#7e57c2} $icon %{-u}%{A}" > "/tmp/lemon/panel_fifo"
