...
weather() {
	...
	printf "%s\n" "weather%{A:x-terminal-emulator -e dash -c 'sleep 0.1 && wmctrl -r \:ACTIVE\: -b add,fullscreen && lemon_weather_launcher.sh' &:}%{U#f06292}%{+u}%{F#f06292} $icon %{F#FFFFFF}$text %{-u}%{A}"
}
...
while :; do        battery 50; sleep 50s; done > "$fifo" &
